...
export default function ScrollableProjects({ projects }: ScrollableProjectsProps) {
  ...
  const scrollToIndex = (index: number) => {
    if (containerRef.current) {
      const newIndex = Math.max(0, Math.min(index, projects.length - 3));
    ...
    <div className="relative">
      ...
      >
        {projects.map((project, index) => (
          <motion.div
            key={index}
            className="flex-none w-full md:w-[calc(33.333%-1.33rem)] group relative overflow-hidden rounded-lg"
